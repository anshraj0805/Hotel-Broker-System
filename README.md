...
## Database Design
This is a database course final project, so obviously the database design is the most important aspect of the project. Three steps are included here, I start by showing our ER-Diagram or the conceptual modeling. I then move to the relational model converting it from the ERD and finally we normalize it several levels to reach the normalized relational model we implement.
...
I list below our core choices to build the web-based application:
...
Next, I list the file structure of the application for convience:
...
Lastly, I mention that we have built wire-frames for the whole website design taht we worked on and it can be found in [this folder](docs/wire-frames/).
